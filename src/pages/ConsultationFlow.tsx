...
import { BookOpen, Check, CheckCircle2, ChevronDown, ChevronRight, Loader2, Lock, MapPin } from 'lucide-react';
...
import BasementBookingFlow from './consultation/BasementBookingFlow';
import {
  AddToCalendar,
  Choice,
  ErrorNote,
...
type Program = {
  ...
  /**
   * Where the calendar sits. 'calendar_early' hands the whole journey to
   * BasementBookingFlow — see lib/program-config.ts. Absent on an older payload
   * ⇒ the original order, which is what every other program still uses.
   */
...
export default function ConsultationFlow() {
  ...
  // The basement flow's screens are in a different ORDER, not a different
  // style: the calendar comes second, and the questions that used to precede it
  // are asked after the slot is held. It gets its own component rather than
  // another set of branches in here — this file already carries two address
  // placements and a conditional explainer screen, and a third shape woven
  // through the same state machine is how the grant flows would eventually
  // break for a reason nobody could see.
  //
  // Placed after the closed check above, so a closed program still shows its
  // closure notice rather than a booking calendar.
  if (program.bookingFlow === 'calendar_early') {
    return <BasementBookingFlow program={program} trafficSource={trafficSource} />;
  }
