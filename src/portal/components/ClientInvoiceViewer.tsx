...
import { buildClientInvoicePdf, type ClientInvoiceData } from './ClientInvoice';

/** Read-only re-view of a client invoice, rebuilt from its ledger snapshot. */
export default function ClientInvoiceViewer({ data, onClose }: { data: ClientInvoiceData; onClose: () => void }) {
  ...
  const download = () => buildClientInvoicePdf(letterhead, data).save(`Invoice ${data.invoiceNumber} - ${clientLabel}.pdf`);
